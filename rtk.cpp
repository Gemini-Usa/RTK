...
{
    res.PDOP = 0;
    for (int i = 0; i < 3; ++i) res.PDOP += Q[i*par+i];
              ...
              DDObs &dd_obs, double *a, double *Q_a)
{
    ...
    SatRes b_sat_1[2]{*find_by_sat(b_sat_res, MAXSATNUM, dd_obs.ref_prn[0], NavSys::GPS),
                      *find_by_sat(b_sat_res, MAXSATNUM, dd_obs.ref_prn[1], NavSys::BDS)};
    SatRes r_sat_1[2]{*find_by_sat(r_sat_res, MAXSATNUM, dd_obs.ref_prn[0], NavSys::GPS),
                      *find_by_sat(r_sat_res, MAXSATNUM, dd_obs.ref_prn[1], NavSys::BDS)};
              ...
              DDObs &dd_obs)
{
    ...
    double p_cor[2][4]{{p[0]/2.0,p[0]/2.0,p[0]/2.0/0.001,p[0]/2.0/0.001},
                       {p[1]/2.0,p[1]/2.0,p[1]/2.0/0.001,p[1]/2.0/0.001}};
    auto W = std::make_unique<double[]>(4*sat_num);
    auto B = std::make_unique<double[]>(4*sat_num*3);
    auto P = std::make_unique<double[]>(4*sat_num*4*sat_num);
    ...
    while (Norm(x.get(), 3) > 1.0E-6 && ite < 10) {
        for (auto& [prn, sys] : dd_sat_view) {
            ...
            W[4*row] = sd->dP[0] - sd_1[sys].dP[0] - dd_rho;
            W[4*row+1] = sd->dP[1] - sd_1[sys].dP[1] - dd_rho;
            W[4*row+2] = (sd->dL[0] - sd_1[sys].dL[0] - dd_obs.fixed_amb[2*row]) * lam[sys][0] - dd_rho;
            W[4*row+3] = (sd->dL[1] - sd_1[sys].dL[1] - dd_obs.fixed_amb[2*row+1]) * lam[sys][1] - dd_rho;
            for (int j = 0; j < 4; ++j) {
                B[(4*row+j)*3] = -sd_los[0];
                B[(4*row+j)*3+1] = -sd_los[1];
                B[(4*row+j)*3+2] = -sd_los[2];
            }
            ++row;
        }
        LSE(B.get(), P.get(), W.get(), 4*sat_num, 3, x.get(), Q.get());
        for (int i = 0; i < 3; ++i) r_r0.dxyz[i] += x[i];
        ++ite;
        row = 0;
    }
    if (ite == 10) return false;
    r_res.rcvpos = r_r0;
    AccuracyAssess(W.get(), P.get(), 4*sat_num, 3, r_res);
